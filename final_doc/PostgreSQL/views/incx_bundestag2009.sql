CREATE OR REPLACE FUNCTION db.getzweitbt2009 () 
RETURNS TABLE (id integer) AS $$
	...
	create temporary table pml as (SELECT * FROM db.parteimandateproland2009 pm WHERE pm.sitze != 0);
	create temporary table dirm as (SELECT * FROM db.direktmandate2009);
	...
	LEFT JOIN db.parteien p ON p.id = b.fkpartei);
        ...
        FOREACH listeid IN ARRAY array(SELECT lp.id FROM db.landeslisten lp JOIN db.legaleparteien2009 l ON lp.fkpartei = l.id)
        LOOP

        fill = (SELECT anzliste 
        FROM listerest lr 
        JOIN db.landeslisten ll ON ll.fkbundesland = lr.bl_id AND ll.fkpartei = lr.partei_id AND ll.jahr = 2009
        WHERE ll.id = listeid);
...
CREATE OR REPLACE VIEW db.bundestag2009 AS(SELECT * FROM (

(SELECT b.id, b.nachname, b.vorname,p.name, 'ZWEIT' as text FROM db.getzweitbt2009() dz 
JOIN db.bewerber b ON dz.id = b.id
JOIN db.parteien p on p.id = b.fkpartei)

UNION 

(SELECT b.id, b.nachname, b.vorname,p.name, 'ERST' as text
FROM db.direktmandate2009 dirma
