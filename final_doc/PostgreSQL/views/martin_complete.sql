CREATE OR REPLACE FUNCTION db.initTempT() 
RETURNS VOID AS $$
BEGIN
	-- zweitstimmen pro bundesland
	create temp table IF NOT EXISTS zweitstimmen AS (SELECT p.id, sum(ez.stimmen), ez.jahr AS total
	FROM db.parteien p
	...
	create temp table IF NOT EXISTS numDirektmandate AS (SELECT * FROM db.getDirectSeatsPerParty());

	create temp table IF NOT EXISTS legaleparteien AS (SELECT * FROM db.getLegalParties(2013) UNION SELECT * FROM db.getLegalParties(2009));
...
CREATE OR REPLACE FUNCTION db.getzweitbt (year_e integer) 
RETURNS TABLE (id integer) AS $$
	...
	create temp table if not exists listerest AS (SELECT bl_id , partei_id, (sitze-
	(SELECT count(*) FROM direktmandate dirm JOIN db.wahlkreise wk ON dirm.wahlkreis_id = wk.id 
	WHERE bl_id = wk.fkbundesland AND pml.partei_id = dirm.partei_id And dirm.year_elec = year_e)) as anzliste FROM db.mandateproland(year_e) pml WHERE pml.year_elec = year_e);

	create temp table if not exists direkt as (SELECT b.id, b.vorname, b.nachname, p.name FROM direktmandate dirm 
	LEFT JOIN db.bewerber b ON b.id = dirm.bewerber_id
	LEFT JOIN db.parteien p ON p.id = b.fkpartei);

        drop table res;
        create temp table if not exists res (id integer);
        
        -- START OF LOGIC

        FOREACH listeid IN ARRAY array(SELECT lp.id FROM db.landeslisten lp 
        JOIN legaleparteien l ON lp.fkpartei = l.partei_id AND lp.jahr = year_e)
        LOOP

        fill = (SELECT anzliste 
        FROM listerest lr 
        JOIN db.landeslisten ll ON ll.fkbundesland = lr.bl_id AND ll.fkpartei = lr.partei_id AND ll.jahr = year_e
        WHERE ll.id = listeid);
...
WITH bundestag AS (SELECT * FROM (

(SELECT b.id, b.nachname, b.vorname,p.name, 'ZWEIT' FROM db.getzweitbt(2013) dz 
...
FROM db.direktmandate dirma
JOIN db.bewerber b ON dirma.bewerber_id = b.id
JOIN db.parteien p on p.id = b.fkpartei
WHERE dirma.year_elec = 2013)
